...
class Accounts:
    ...
    async def get_accounts(self):
        ...
        if not available_accounts:
            raise ValueError("Khong co san tai khoan!")
        else:
            logger.success(f"Tim kiem tai khoan co san: {len(available_accounts)}.")
        ...
        if not valid_accounts:
            raise ValueError("Khong co phien hop le")
        else:
            return valid_accounts

    async def create_sessions(self):
        while True:
            session_name = input('\nNhap ten phien (nhan Enter de thoat): ')
            ...
            if config.PROXY['USE_PROXY_FROM_FILE']:
                ...
                proxy = input("Nhap proxy theo dinh dang login:password@ip:port (nhan Enter de su dung ma khong can proxy): ")

            dict_proxy = self.parse_proxy(proxy) if proxy else None

            phone_number = (input("Nhap sdt caa tai khoan: ")).replace(' ', '')
            ...
            logger.success(f'Da them tai khoan {me.username} ({me.first_name}) | {me.phone_number}')